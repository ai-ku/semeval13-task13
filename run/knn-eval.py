...
parser.add_option("-i", "--inst_file", dest="inst_file", default=None,
        help="Instance file. Rows (0...100) matched with instance ids", 
...
def eval_knn(source, gold_file, inst_file, k, is_weighted):

    #print k, is_weighted, gold_file, inst_file
    #exit()
    
    keys = map(str.strip, open(inst_file).readlines())
    dsense = gold_process(gold_file)
    source_lines = file_open(source).readlines()

    if not (len(keys) == len(dsense.keys()) == len(source_lines)):
        print >> sys.stderr, "lengths do not match", 
        print >> sys.stderr, len(source_lines), len(keys), len(dsense.keys())
        print >> sys.stderr, source, gold_file, inst_file, k, is_weighted
        exit(-1)

    for line in source_lines:
        line = line.split()
        curr_inst = keys[int(line[0])]
        neighbors = map(int, line[1::2])
        neighbors = neighbors[:k]
        neighbors = [keys[n] for n in neighbors]
        weights = None
        if is_weighted:
