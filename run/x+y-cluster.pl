...
my $kmeans = "x+y2kmeans.py -p pairs.100.gz | wkmeans -r 128 -l -w -v -s $seed -k ";
...
#    print join(" | ", $input, $filter, $scode, $column, $km, "gzip > $tmp/km$key.gz")." &\n";
    ...
    $process_all .= join(" | ", $input, $filter, $scode, $km, "gzip > $tmp/km$key.gz")." & ";
}

system($process_all."wait");

foreach my $key (keys %targets) {
    system("zcat pairs.100.gz | grep -P '^<$key.[^\\d]' | x+y-find-sense.py $tmp/km$key.gz >> $tmp/out.key");
}
